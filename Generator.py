...
from Globals import TestMaster, TradeIdGen
...
def resetPrecondition():
    ...
    ob.enterOrder(Order(0,1, 14, None, None, 5, FillTypeEnum.NORMAL, PriceTypeEnum.LIMIT, isp, isp_end, expiration))
    ob.enterOrder(Order(0,1, 18, None, None, 5, FillTypeEnum.NORMAL, PriceTypeEnum.LIMIT,isp, isp_end, expiration))
    ob.enterOrder(Order(0,2, 12, None, None, 5, FillTypeEnum.NORMAL, PriceTypeEnum.LIMIT, isp, isp_end, expiration))
    ob.enterOrder(Order(0,3, None, 10, 27, 5, FillTypeEnum.NORMAL, PriceTypeEnum.LIMIT, isp, isp_end, expiration))
    ob.enterOrder(Order(0,4, None, 10, 28, 5, FillTypeEnum.NORMAL, PriceTypeEnum.LIMIT, isp, isp_end, expiration))
    ob.enterOrder(Order(0,5, None, 5, 37,  5, FillTypeEnum.NORMAL, PriceTypeEnum.LIMIT,isp, isp_end, expiration))
    return ob
...
if __name__== "__main__":
    ...
    participant=6  # Agressor participant (6). Participants 1..5 are already given on the passive precondition orders

    genTest("TEST ORDER", Order(0,participant, None, 5, 5, 4, FillTypeEnum.NORMAL, PriceTypeEnum.LIMIT, isp, isp_end, expiration))
